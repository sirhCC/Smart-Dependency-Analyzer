...
exports.runSecurityValidationTests = runSecurityValidationTests;
exports.testMaliciousPackageDetection = testMaliciousPackageDetection;
exports.testSupplyChainAttackDetection = testSupplyChainAttackDetection;
exports.testThreatIntelligenceExtraction = testThreatIntelligenceExtraction;
exports.testPerformanceUnderAttack = testPerformanceUnderAttack;
exports.createMaliciousTestData = createMaliciousTestData;
 ...
 */
async function testMaliciousPackageDetection() {
    ...
    for (const pkg of maliciousPackages) {
        ...
        try {
            ...
            const foundPatterns = suspiciousPatterns.filter(pattern => packageText.toLowerCase().includes(pattern));
            ...
            if (pkg.scripts) {
                const suspiciousScripts = Object.entries(pkg.scripts).filter(([key, script]) => {
                    return key.includes('install') && (script.includes('curl') ||
            ...
            if (pkg.maintainers) {
                const suspiciousMaintainers = pkg.maintainers.filter(m => m.email?.includes('temp') ||
    ...
    for (const pkg of vulnerablePackages) {
        ...
        try {
            ...
            if (prediction) {
                ...
                const matchingVuln = realVulnerabilities.find(v => v.affectedVersions.some(range => pkg.version && range.includes(pkg.version?.split('.')[0] || '')));
                if (matchingVuln) {
 ...
 */
async function testSupplyChainAttackDetection() {
    ...
    for (const pkg of supplyChainAttacks) {
        ...
        try {
            ...
            const potentialTyposquat = similarPackages.find(legitimate => levenshteinDistance(pkg.name, legitimate) <= 2 && pkg.name !== legitimate);
            ...
            const newMaintainers = pkg.maintainers?.filter(m => m.name?.includes('2023') ||
            ...
            const suspiciousDeps = Array.from(pkg.dependencies?.keys() || []).filter(dep => dep.includes('analytics') ||
 ...
 */
async function testThreatIntelligenceExtraction() {
 ...
 */
async function testPerformanceUnderAttack() {
    ...
    try {
        ...
        const maliciousDetected = highRiskDetections.filter(p => maliciousPackages.some(m => m.name === p.packageName));
...
//# sourceMappingURL=ai-security-validation-test.js.map