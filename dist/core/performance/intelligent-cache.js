...
const logger = (0, logger_1.getLogger)('IntelligentCache');
/**
...
class IntelligentCache extends events_1.EventEmitter {
    ...
    async clear() {
        try {
            ...
            Object.keys(this.stats).forEach(level => {
                Object.assign(this.stats[level], {
                    hits: 0,
                    misses: 0,
                    entries: 0,
                    hitRate: 0,
    ...
    async setToL1(key, value, ttl = this.config.defaultTtl) {
        const size = this.estimateSize(value);
        // Ensure we have space
        while (this.currentMemoryUsage + size > this.config.maxMemorySize * 1024 * 1024 ||
    ...
    async setToL2(key, value, ttl = this.config.defaultTtl) {
        if (!this.redisClient)
            return;
        try {
    ...
    async getFromL3(_key) {
        // File cache implementation placeholder
        return null;
    }
    async setToL3(_key, _value, _ttl) {
        // File cache implementation placeholder
    }
    evictLRU() {
    ...
    async removeFromFileCache(_key) {
