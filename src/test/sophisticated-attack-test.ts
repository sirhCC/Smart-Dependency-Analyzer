 ...
 */
function createSophisticatedAttackData(): Package[] {
  return [
    {
 ...
 */
async function testSophisticatedAttacks(): Promise<void> {
  ...
  const maliciousDetected = predictions.filter(p => 
    maliciousPackages.includes(p.packageName) && p.riskScore >= 80
  ).length;
