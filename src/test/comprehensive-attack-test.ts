 ...
 */
function createComprehensiveAttackVectors(): Package[] {
  return [
 ...
 */
async function testComprehensiveAttackDetection(): Promise<void> {
  ...
  const attackResults: { [key: string]: { detected: boolean; score: number; type: string } } = {};
  ...
  for (const prediction of predictions) {
    const attackType = attackTypes[prediction.packageName as keyof typeof attackTypes];
