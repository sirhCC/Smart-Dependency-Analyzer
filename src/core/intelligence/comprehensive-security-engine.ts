...
export class ComprehensiveSecurityEngine {
  ...
  private async performComprehensiveAnalysis(pkg: Package): Promise<VulnerabilityPrediction> {
    ...
    if (riskScore >= 90) severity = VulnerabilitySeverity.CRITICAL;
    else if (riskScore >= 70) severity = VulnerabilitySeverity.HIGH;
    else if (riskScore >= 50) severity = VulnerabilitySeverity.MEDIUM;
