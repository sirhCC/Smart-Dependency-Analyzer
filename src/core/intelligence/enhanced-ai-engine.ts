...
const logger = getLogger('EnhancedAIEngine');
...
export class EnhancedAIEngine {
   ...
   */
  private async analyzePackageForThreats(pkg: Package): Promise<VulnerabilityPrediction> {
    const risks: string[] = [];
    ...
    if (risks.length > 5) confidence = 0.95;
    else if (risks.length > 3) confidence = 0.90;
    else if (risks.length > 1) confidence = 0.85;

    // Determine severity
    let severity = VulnerabilitySeverity.LOW;
    if (riskScore >= 80) severity = VulnerabilitySeverity.CRITICAL;
    else if (riskScore >= 60) severity = VulnerabilitySeverity.HIGH;
    else if (riskScore >= 40) severity = VulnerabilitySeverity.MEDIUM;
   ...
   */
  private analyzeScripts(pkg: Package): { score: number; factors: string[]; mitigations: string[] } {
   ...
   */
  private analyzeAuthors(pkg: Package): { score: number; factors: string[]; mitigations: string[] } {
    const factors: string[] = [];
    const mitigations: string[] = [];
    let score = 0;
    ...
    if (pkg.maintainers && pkg.maintainers.length > 0) {
      ...
      const recentMaintainers = pkg.maintainers.filter(m => 
        m.name?.match(/(?:new|recent|2023|2024|2025)/i)
      );
   ...
   */
  private analyzeTyposquatting(pkg: Package): { score: number; factors: string[]; mitigations: string[] } {
    const factors: string[] = [];
    const mitigations: string[] = [];
    let score = 0;
   ...
   */
  private analyzeMetadata(pkg: Package): { score: number; factors: string[]; mitigations: string[] } {
    const factors: string[] = [];
    const mitigations: string[] = [];
    let score = 0;
    ...
    if (pkg.keywords) {
      ...
      const foundSuspicious = pkg.keywords.filter(kw => 
        suspiciousKeywords.some(suspicious => kw.toLowerCase().includes(suspicious))
      );
   ...
   */
  private analyzeDependencies(pkg: Package): { score: number; factors: string[]; mitigations: string[] } {
    const factors: string[] = [];
    const mitigations: string[] = [];
    let score = 0;
   ...
   */
  private performMLAnalysis(pkg: Package): { score: number } {
    ...
    if (pkg.publishedAt) {
      const ageInDays = (Date.now() - pkg.publishedAt.getTime()) / (1000 * 60 * 60 * 24);
      if (ageInDays < 30) score += 10; // New packages are riskier
    }

    // Factor in popularity
    if (pkg.downloadCount) {
      if (pkg.downloadCount < 1000) score += 15; // Unpopular packages are riskier
      if (pkg.downloadCount > 10000000) score -= 10; // Very popular packages are generally safer
    }
